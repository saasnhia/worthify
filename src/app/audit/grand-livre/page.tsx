...
const CLASSE_COLORS: Record<number, string> = {
  1: 'bg-purple-500/15 text-purple-300',
  2: 'bg-blue-500/15 text-blue-300',
  3: 'bg-orange-500/15 text-orange-300',
  4: 'bg-amber-500/15 text-amber-300',
  5: 'bg-cyan-500/15 text-cyan-300',
  6: 'bg-coral-500/15 text-coral-300',
  7: 'bg-emerald-500/15 text-emerald-300',
}
    ...
    <AppShell>
      <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-400 mb-1">
              ...
              {mode === 'detail' ? (
                <>
                  ...
                  <span>/</span>
                  <span className="text-gray-200">{selectedCompte}</span>
                </>
              ) : (
                <span className="text-gray-200">Grand Livre</span>
              )}
            </div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              ...
              {mode === 'detail' ? (
                <>
                  ...
                  <span className="text-lg font-normal text-gray-300">— {selectedCompteLib}</span>
                </>
              ) : (
                'Grand Livre'
              )}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-1">
              ...
              <input
                ...
                className="bg-gray-800 border border-gray-600 text-gray-200 text-sm rounded-lg px-2 py-1.5"
              />
              ...
              <>
                ...
                <div className="relative flex-1 min-w-[200px]">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-1.5 bg-gray-800 border border-gray-600 text-gray-200 text-sm rounded-lg"
                  />
                </div>
              </>
            )}
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <Card>
            <p className="text-xs text-gray-400 mb-0.5">Total Débit</p>
            <p className="text-lg font-bold font-mono text-white">{formatEuro(totals.debit)}</p>
          </Card>
          <Card>
            <p className="text-xs text-gray-400 mb-0.5">Total Crédit</p>
            <p className="text-lg font-bold font-mono text-white">{formatEuro(totals.credit)}</p>
          </Card>
          <Card>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-700 bg-gray-800/50">
                      <th className="px-4 py-2.5 text-left text-xs text-gray-400 font-bold">Compte</th>
                      <th className="px-4 py-2.5 text-left text-xs text-gray-400 font-bold">Libellé</th>
                      <th className="px-4 py-2.5 text-center text-xs text-gray-400 font-bold">Classe</th>
                      <th className="px-4 py-2.5 text-right text-xs text-gray-400 font-bold">Total Débit</th>
                      <th className="px-4 py-2.5 text-right text-xs text-gray-400 font-bold">Total Crédit</th>
                      <th className="px-4 py-2.5 text-right text-xs text-gray-400 font-bold">Solde</th>
                      <th className="px-4 py-2.5 text-center text-xs text-gray-400 font-bold">Écritures</th>
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {filteredComptes.map(c => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5 text-gray-200 text-xs">{c.compte_lib}</td>
                        ...
                        <td className="px-4 py-2.5 text-right font-mono text-xs text-gray-200">{formatEuro(c.total_debit)}</td>
                        <td className="px-4 py-2.5 text-right font-mono text-xs text-gray-200">{formatEuro(c.total_credit)}</td>
                        <td className={`px-4 py-2.5 text-right font-mono text-xs font-medium ${c.solde >= 0 ? 'text-emerald-400' : 'text-coral-400'}`}>
                          {formatEuro(c.solde)}
                        </td>
                        <td className="px-4 py-2.5 text-center text-xs text-gray-400">{c.nb_ecritures}</td>
                  ...
                  <tfoot>
                    <tr className="border-t border-gray-600 bg-gray-800">
                      <td colSpan={3} className="px-4 py-2.5 text-xs font-semibold text-gray-300 text-right">
                        {filteredComptes.length} comptes
                      </td>
                      <td className="px-4 py-2.5 text-right font-mono text-xs font-bold text-white">{formatEuro(totals.debit)}</td>
                      <td className="px-4 py-2.5 text-right font-mono text-xs font-bold text-white">{formatEuro(totals.credit)}</td>
            ...
            lignes.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-700 bg-gray-800/50">
                      <th className="px-3 py-2.5 text-left text-xs text-gray-400 font-bold">Date</th>
                      <th className="px-3 py-2.5 text-left text-xs text-gray-400 font-bold">N°</th>
                      <th className="px-3 py-2.5 text-left text-xs text-gray-400 font-bold">Journal</th>
                      <th className="px-3 py-2.5 text-left text-xs text-gray-400 font-bold">Libellé</th>
                      <th className="px-3 py-2.5 text-left text-xs text-gray-400 font-bold">Pièce</th>
                      <th className="px-3 py-2.5 text-left text-xs text-gray-400 font-bold">Lettrage</th>
                      <th className="px-3 py-2.5 text-right text-xs text-gray-400 font-bold">Débit</th>
                      <th className="px-3 py-2.5 text-right text-xs text-gray-400 font-bold">Crédit</th>
                      <th className="px-3 py-2.5 text-right text-xs text-gray-400 font-bold">Solde cumulé</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-700">
                    {lignes.map(l => (
                      <tr key={l.id} className="hover:bg-gray-800/50 transition-colors">
                        <td className="px-3 py-2 text-gray-300 text-xs font-mono">{formatDate(l.date_ecriture)}</td>
                        <td className="px-3 py-2 text-gray-200 text-xs font-mono">{l.ecriture_num}</td>
                        <td className="px-3 py-2 text-gray-300 text-xs">{l.journal_code}</td>
                        <td className="px-3 py-2 text-gray-200 text-xs max-w-[250px] truncate">{l.libelle}</td>
                        <td className="px-3 py-2 text-gray-400 text-xs font-mono">{l.piece_ref ?? '-'}</td>
                        <td className="px-3 py-2 text-gray-400 text-xs font-mono">{l.lettrage ?? ''}</td>
                        <td className="px-3 py-2 text-right font-mono text-xs">
                          {Number(l.debit) > 0 ? <span className="text-emerald-300">{formatEuro(Number(l.debit))}</span> : <span className="text-gray-600">-</span>}
                        </td>
                        <td className="px-3 py-2 text-right font-mono text-xs">
                          {Number(l.credit) > 0 ? <span className="text-rose-300">{formatEuro(Number(l.credit))}</span> : <span className="text-gray-600">-</span>}
                        </td>
                  ...
                  <tfoot>
                    <tr className="border-t border-gray-600 bg-gray-800">
                      <td colSpan={6} className="px-3 py-2.5 text-xs font-semibold text-gray-300 text-right">
                        Totaux
                      </td>
                      <td className="px-3 py-2.5 text-right font-mono text-xs font-bold text-white">{formatEuro(totals.debit)}</td>
                      <td className="px-3 py-2.5 text-right font-mono text-xs font-bold text-white">{formatEuro(totals.credit)}</td>
